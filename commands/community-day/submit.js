		...
		(async function() {
			...
			if (dateValidation == 'early' || dateValidation == 'late') {
				...
				return message.channel.send(
					`<@${user.id}>, community day submissions are ${closedOpens}.`,
				);
			}
			...
			if (!userInfo) {
				return message.channel.send(
					`<@${user.id}>, you are not registered for this Community Day event, please be sure to submit your registration before the deadline. This community day deadline was ${cdConfig.enrollmentEnd}.`,
				);
			}
			...
			if (attachments.length == 0) {
				return message.channel.send(
					`<@${user.id}> please upload a screeshot of your submission with the appraisal window open.`,
				);
			}
			...
			sequences
				.then(() => {
					...
					if (!userInfo.weight) {
						if (
							...
							message.channel.send(
								`<@${user.id}> could not read weight, please retry or tag a manager.`,
							);
						}
					}
					...
					if (!userInfo.height) {
						if (
							...
							message.channel.send(
								`<@${user.id}> could not read height, please retry or tag a manager.`,
							);
						}
					}
				...
				.catch((err) => {
					...
					message.channel.send(
						'An error has occurred, please resubmit. This error is normally due to your image files being to large for the API to process. (5Mb limit)',
					);
				});
		})().catch((error) => {
